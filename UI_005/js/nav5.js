...
/**
 화면의 nav의 요소들인 li tag 를 클릭했을 때 반응하는
 event handler 를 설정하고싶다
 각각의 li tag 를 querySelector () 하여 개별적인 event handler 설정할 수 있다
 하지만 li tag 의 개수가 많아지만 그만큼 코드의 글이가 길어진다
 또한 유사한 코드가 중복되어 작성된다
 
 이럴때 li tag 의 부모 tag 요소에 event handler 를 설정하고
 event bubbling 을 역으로 잉요하여
 event 처리를 간소화 시킨다
 
 nav.mail_nav tag 에 event 를 설정하고
